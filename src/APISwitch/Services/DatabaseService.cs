...
CREATE TABLE IF NOT EXISTS Providers (
    ...
    public List<Provider> GetProviders(int toolType)
    {
        lock (_syncRoot)
        {
            ...
            using var command = connection.CreateCommand();
            command.CommandText = @"
SELECT Id, ToolType, Name, BaseUrl, ApiKey, IsActive, SortOrder
FROM Providers
...
ORDER BY SortOrder ASC, Id ASC;";
    ...
    public int AddProvider(Provider provider)
    {
        lock (_syncRoot)
        {
            ...
            using var command = connection.CreateCommand();
            command.CommandText = @"
INSERT INTO Providers (ToolType, Name, BaseUrl, ApiKey, IsActive, SortOrder)
VALUES ($toolType, $name, $baseUrl, $apiKey, $isActive, $sortOrder);
