...
## Purpose
TBD - created by archiving change provider-dialog-model-list. Update Purpose after archive.
## Requirements
...
系统 SHALL 使用当前供应商配置的 `ApiKey` 请求模型列表，并按工具类型复用 API 测试请求头。

#### Scenario: 发起模型列表请求
- **WHEN** 用户在编辑页面点击“获取模型”
- **THEN** 系统向目标接口发送请求，并携带该供应商对应 API 测试请求所用的请求头
